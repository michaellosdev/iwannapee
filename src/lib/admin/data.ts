...
export async function getAdminDashboardData() {
  ...
  const [profilesResult, usersResult, restroomsResult, updatesResult, reportsResult, campaignsResult, photosResult, notesResult] = await Promise.all([
    ...
    admin.from("advertising_campaigns").select("id,created_by,business_name,restroom_name,address,headline,offer_text,price_cents,placement_bid_cents,support_amount_cents,status,is_test,stripe_payment_intent_id,starts_at,ends_at,stopped_at,deleted_at,refund_requested_at,payment_refunded_at,created_at").order("created_at", { ascending: false }).limit(100),
